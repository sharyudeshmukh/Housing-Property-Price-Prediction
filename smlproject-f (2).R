...
#########Lotfrontage test
z=l %>% select(LotFrontage,Neighborhood) %>%
  group_by(Neighborhood)%>% summarize(median=median(LotFrontage,na.rm=TRUE))

t=l[which(is.na(l$LotFrontage)),] %>% select(LotFrontage,Neighborhood)
q=left_join(t,z,by="Neighborhood")
...
plot(summary(rf1)) 
h2o.varimp_plot(rf1)
(h2o.performance(rf1, newdata = valid)) 
...
sortedGrid <- h2o.getGrid("final_grid", sort_by = "RMSE")    
sortedGrid
for (i in 1:5) {
...
predictions= h2o.predict(model, newdata=test1)

setwd("/Users/shivayogibiradar/Desktop/kaggle submissions")
write.csv(as.data.frame(predictions),file="ensemble.csv")
...
test1=as.h2o(testf)
h2o.performance(rf1, newdata = test)
